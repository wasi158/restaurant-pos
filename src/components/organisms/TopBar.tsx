...
import { Search, Bell, Sun, Moon, Cloud, Minus, Square, X, Check, Menu } from 'lucide-react';
import { useElectron } from '../../hooks/useElectron';
import { motion, AnimatePresence } from 'motion/react';
import { useAppSettings } from '../../lib/appSettings';
import { usePos } from '../../lib/pos/store';
import { getExpiringItems, expiryLabel, getExpiryStatus } from '../../lib/pos/expiry';

const STATIC_NOTIFS = [
  { id: 'n-1', title: 'New VIP Reservation', desc: 'Table 14 booked for 8 PM by Mr. Sterling.', time: '1h ago', unread: true, type: 'info' },
  { id: 'n-2', title: 'Shift Update', desc: 'Sarah clocked in 15 mins late.', time: '2h ago', unread: false, type: 'default' },
];
...
export function TopBar({ theme, toggleTheme, onMenuClick }: TopBarProps) {
  ...
  const expiryNotifs = useMemo(() => {
    return getExpiringItems(state.inventory)
      .filter(i => !dismissedExpiry.has(i.id))
      .map(item => ({
        id: `exp-${item.id}`,
        title: getExpiryStatus(item) === 'expired' ? 'Expired Product' : 'Expiring Soon',
        desc: `${item.name} — ${expiryLabel(item)}`,
        time: 'Now',
        unread: true,
        type: getExpiryStatus(item) === 'expired' ? 'danger' : 'warning',
      }));
  }, [state.inventory, dismissedExpiry]);

  const notifs = [...expiryNotifs, ...STATIC_NOTIFS];
  ...
  const markAllRead = () => {
    const expiryIds = expiryNotifs.map(n => n.id.replace('exp-', ''));
    setDismissedExpiry(prev => new Set([...prev, ...expiryIds]));
  };
  ...
  return (
    <header className="h-14 bg-surface-container-low/95 backdrop-blur-xl flex items-center justify-between px-5 border-b border-outline-variant sticky top-0 z-40 drag-region shrink-0">
      ...
      {/* Mobile menu + Search */}
      <div className="flex items-center gap-2 flex-1 no-drag min-w-0">
        <button
          type="button"
          onClick={onMenuClick}
          className="lg:hidden w-9 h-9 flex items-center justify-center rounded-xl text-on-surface-variant hover:text-primary hover:bg-primary/15 transition-all shrink-0"
      ...
      <div className="flex items-center gap-2 no-drag">
          ...
          <AnimatePresence>
            {showNotifs && (
              <motion.div 
                ...
                className="absolute right-0 top-full mt-2 w-[calc(100vw-2rem)] sm:w-80 max-w-80 bg-surface-container-low border border-outline-variant shadow-2xl rounded-2xl overflow-hidden z-50 text-left"
              >
                ...
                <div className="max-h-80 overflow-y-auto">
                  {notifs.map(n => (
                    <div 
                      key={n.id} 
                      onClick={() => {
                        if (n.id.startsWith('exp-')) {
                          setDismissedExpiry(prev => new Set([...prev, n.id.replace('exp-', '')]));
                        }
                      }}
                      className={`p-4 border-b border-outline-variant last:border-0 hover:bg-surface-container-high transition-colors cursor-pointer ${n.unread ? (n.type === 'danger' ? 'bg-error/5' : n.type === 'warning' ? 'bg-tertiary/5' : 'bg-primary/5') : ''}`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <h4 className={`text-xs font-bold leading-tight ${n.type === 'danger' ? 'text-error' : n.type === 'warning' ? 'text-tertiary' : n.unread ? 'text-on-surface' : 'text-on-surface-variant'}`}>{n.title}</h4>
        ...
        <div className="hidden md:block h-5 w-px bg-outline-variant mx-1" />
        ...
        {/* Window controls — Electron only, hidden on mobile */}
        <div className="hidden md:flex items-center ml-1 pl-3 border-l border-outline-variant gap-0.5">
