...
import { TrendingUp, TrendingDown, DollarSign, Users, ShoppingBag, Clock, Download, CheckCircle2, Scale, Receipt } from 'lucide-react';
...
const PERIODS = reportsJson.periods as readonly string[];
type Period = (typeof PERIODS)[number];
const REPORTS_DATA = reportsJson.data as Record<string, any>;
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 overflow-y-auto min-h-full">
      <PageHeader
        ...
        subtitle={`Amounts use ${currency} (Settings → Restaurant Profile). Demo charts use sample data; Profit and loss below is from real POS orders.`}
        actions={
          <div className="flex items-center gap-3">
            <CategoryFilter categories={[...PERIODS]} active={period} onChange={(p) => setPeriod(p)} />
            <Button
              variant={exported ? 'secondary' : 'outline'}
              onClick={handleExport}
              icon={exported ? <CheckCircle2 className="w-4 h-4" /> : <Download className="w-4 h-4" />}
              className={exported ? 'bg-secondary/15 border-secondary/30 text-secondary' : ''}
            >
      ...
      <AnimatePresence mode="wait">
        <motion.div key={period + '-kpi'} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}
          ...
          {kpis.map((k, i) => (
            <StatCard key={i} label={k.label} value={k.value} change={k.val} up={k.up} icon={<k.icon className="w-4 h-4" />} />
          ))}
        </motion.div>
      </AnimatePresence>
